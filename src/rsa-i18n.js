...
const RSAi18n = (function () {
  ...
  const STR = {
    ...
    'tab.advisor':    { en: 'Policy advisor', fr: 'Conseil en politiques' },
    'adv.title':      { en: 'Policy Advisor — guided self-sufficiency analysis',
                        fr: 'Conseil en politiques — analyse guidée de l’autosuffisance' },
    'adv.lede':       { en: 'what drives self-sufficiency, why a country has not reached it, what would get it there, and who already has',
                        fr: 'ce qui détermine l’autosuffisance, pourquoi un pays ne l’a pas atteinte, ce qu’il faudrait pour y parvenir, et qui y est déjà arrivé' },
    'adv.disclosure': {
      en: 'This is a reasoning engine, not a language model. It decomposes the data with an exact identity, ranks what it finds against stated thresholds, and assembles the explanation from that — so every sentence carries the numbers it came from, the same question always gives the same answer, and nothing here can be confidently wrong about a trend it never looked at. The thresholds are published in the module and can be argued with.',
      fr: 'Il s’agit d’un moteur de raisonnement, et non d’un modèle de langage. Il décompose les données selon une identité exacte, classe ses observations selon des seuils explicites et construit l’explication à partir de là — chaque phrase porte donc les chiffres dont elle provient, une même question donne toujours la même réponse, et rien ici ne peut se tromper avec assurance sur une tendance qu’il n’a jamais examinée. Les seuils sont publiés dans le module et peuvent être discutés.'
    },
    'adv.driversTitle': { en: 'What drives self-sufficiency here',
                          fr: 'Ce qui détermine l’autosuffisance ici' },
    'adv.driversLede': {
      en: 'Self-sufficiency is a race between supply and demand. Because production is area times yield and consumption is population times per-capita use, the movement of the ratio decomposes exactly — not approximately — into four terms. Window: {0}–{1}.',
      fr: 'L’autosuffisance est une course entre l’offre et la demande. La production étant la superficie multipliée par le rendement, et la consommation la population multipliée par la consommation par habitant, l’évolution du ratio se décompose exactement — et non approximativement — en quatre termes. Période : {0}–{1}.'
    },
    'adv.supplyGrowth': { en: 'Supply growth', fr: 'Croissance de l’offre' },
    'adv.demandGrowth': { en: 'Demand growth', fr: 'Croissance de la demande' },
    'adv.netGap':       { en: 'Net (supply − demand)', fr: 'Net (offre − demande)' },
    'adv.term':         { en: 'Term', fr: 'Terme' },
    'adv.side':         { en: 'Side', fr: 'Côté' },
    'adv.supply':       { en: 'Supply', fr: 'Offre' },
    'adv.demand':       { en: 'Demand', fr: 'Demande' },
    'adv.growthRate':   { en: 'Growth', fr: 'Croissance' },
    'adv.contribution': { en: 'Share of movement', fr: 'Part du mouvement' },
    'adv.termsCaption': { en: 'Annualised log growth of each term, and its share of total absolute movement.',
                          fr: 'Croissance logarithmique annualisée de chaque terme et sa part du mouvement absolu total.' },
    'adv.dominant':     { en: 'Largest single term: {0}, accounting for {1}% of all movement in the ratio.',
                          fr: 'Terme dominant : {0}, représentant {1} % de l’ensemble du mouvement du ratio.' },
    'adv.residual':     { en: 'Identity residual: {0} — the decomposition is exact, so this is the platform checking its own arithmetic.',
                          fr: 'Résidu de l’identité : {0} — la décomposition est exacte ; il s’agit donc d’une vérification arithmétique de la plateforme.' },
    'adv.window':       { en: 'Window', fr: 'Période' },
    'adv.recent':       { en: 'recent', fr: 'récente' },
    'adv.fullRecord':   { en: 'full record', fr: 'ensemble de la période' },
    'adv.windowCaption': {
      en: 'The diagnosis below uses the recent window. An endpoint decomposition over sixty years can be exactly right and still mislead: Benin was 15.9% self-sufficient in 1961 and 15.9% in 2024, which reads as "nothing changed" while concealing a rise to 64.6% by 2010 and a collapse back.',
      fr: 'Le diagnostic ci-dessous utilise la période récente. Une décomposition entre deux extrémités sur soixante ans peut être exacte et néanmoins trompeuse : le Bénin était autosuffisant à 15,9 % en 1961 et à 15,9 % en 2024, ce qui donne l’impression que « rien n’a changé » tout en masquant une hausse à 64,6 % en 2010 puis un effondrement.'
    },
    'adv.divergent':    { en: 'The two windows disagree by enough to change the advice. Something changed regime; read the recent one.',
                          fr: 'Les deux périodes divergent suffisamment pour modifier la recommandation. Un changement de régime s’est produit ; retenez la période récente.' },
    'adv.raceTitle':    { en: 'The race: what adds to supply, what adds to demand',
                          fr: 'La course : ce qui accroît l’offre, ce qui accroît la demande' },
    'adv.raceCaption':  { en: 'Demand terms are shown negative, because they consume the same headroom supply creates.',
                          fr: 'Les termes de demande sont affichés en négatif, car ils absorbent la marge que crée l’offre.' },
    'adv.whyTitle':     { en: 'Why self-sufficiency has not been reached',
                          fr: 'Pourquoi l’autosuffisance n’est pas atteinte' },
    'adv.whyLede':      { en: 'Self-sufficiency stands at {0}% ({1}). Ranked causes, most severe first, each with the evidence that raised it and what follows from it.',
                          fr: 'L’autosuffisance s’élève à {0} % ({1}). Causes classées, les plus graves d’abord, chacune accompagnée des données qui l’ont fait apparaître et de ses implications.' },
    'sev.critical':     { en: 'critical', fr: 'critique' },
    'sev.high':         { en: 'high', fr: 'élevé' },
    'sev.medium':       { en: 'medium', fr: 'moyen' },
    'sev.info':         { en: 'context', fr: 'contexte' },
    'sev.good':         { en: 'favourable', fr: 'favorable' },
    'adv.whatTitle':    { en: 'What it would take', fr: 'Ce qu’il faudrait' },
    'adv.whatLede':     { en: 'First the arithmetic of what closes the gap, then the instruments that plausibly deliver it — each with the objection that belongs to it, because an instrument recommended without its caveat is not advice.',
                          fr: 'D’abord l’arithmétique de ce qui comble l’écart, puis les instruments susceptibles d’y parvenir — chacun avec l’objection qui lui revient, car un instrument recommandé sans réserve n’est pas un conseil.' },
    'adv.horizon':      { en: 'Horizon', fr: 'Horizon' },
    'adv.requiredMult': { en: 'Production must rise by', fr: 'La production doit croître de' },
    'adv.leastCostMix': { en: 'Least-cost route', fr: 'Voie de moindre coût' },
    'adv.feasible':     { en: 'Reachable', fr: 'Atteignable' },
    'adv.condCaption':  { en: 'From the Self-Sufficiency Condition module, under its stated agronomic ceilings.',
                          fr: 'Issu du module Condition d’autosuffisance, sous ses plafonds agronomiques déclarés.' },
    'adv.alreadyThere': { en: 'already self-sufficient', fr: 'déjà autosuffisant' },
    'adv.noRoute':      { en: 'no route within the ceilings', fr: 'aucune voie dans les limites' },
    'adv.yes':          { en: 'yes', fr: 'oui' },
    'adv.no':           { en: 'no', fr: 'non' },
    'adv.instruments':  { en: 'Instruments, and what is wrong with each',
                          fr: 'Instruments, et ce qui pose problème dans chacun' },
    'adv.instrument':   { en: 'Instrument', fr: 'Instrument' },
    'adv.whyItHelps':   { en: 'Why it helps', fr: 'Pourquoi cela aide' },
    'adv.caveat':       { en: 'Caveat', fr: 'Réserve' },
    'adv.instrumentsCaption': { en: 'Selected by the diagnosis above, not offered as a menu. The caveats are not boilerplate: each is the reason the instrument fails where it fails.',
                                fr: 'Sélectionnés par le diagnostic ci-dessus, et non proposés comme un menu. Les réserves ne sont pas de pure forme : chacune explique pourquoi l’instrument échoue là où il échoue.' },
    'adv.whoTitle':     { en: 'Who has managed it, and how', fr: 'Qui y est parvenu, et comment' },
    'adv.whoLede':      { en: 'A prescription is more credible when someone has already followed it. These are the countries that are self-sufficient today, and those whose ratio is improving fastest.',
                          fr: 'Une recommandation est plus crédible lorsqu’un pays l’a déjà suivie. Voici les pays aujourd’hui autosuffisants, et ceux dont le ratio progresse le plus vite.' },
    'adv.alreadySS':    { en: 'Self-sufficient today', fr: 'Autosuffisants aujourd’hui' },
    'adv.improvingFastest': { en: 'Improving fastest', fr: 'Progression la plus rapide' },
    'adv.yieldGrowth':  { en: 'Yield growth', fr: 'Croissance du rendement' },
    'adv.areaGrowth':   { en: 'Area growth', fr: 'Croissance des superficies' },
    'adv.ssrTrend':     { en: 'SSR trend', fr: 'Tendance du TAS' },
    'adv.whoCaveat':    { en: 'Self-sufficiency is not the same as food security, and a small producer with a high ratio is not necessarily a model. Egypt is self-sufficient with irrigation the Sahel does not have; Guinea-Bissau is close to it at low yields and low incomes. Read the route, not only the ratio.',
                          fr: 'L’autosuffisance n’équivaut pas à la sécurité alimentaire, et un petit producteur au ratio élevé n’est pas nécessairement un modèle. L’Égypte est autosuffisante grâce à une irrigation dont le Sahel ne dispose pas ; la Guinée-Bissau s’en approche avec de faibles rendements et de faibles revenus. Lisez la trajectoire, pas seulement le ratio.' },
    'adv.dlBrief':      { en: 'Download this analysis (Markdown)',
                          fr: 'Télécharger cette analyse (Markdown)' },
